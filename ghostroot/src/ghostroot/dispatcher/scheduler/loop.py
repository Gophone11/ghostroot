...
from ghostroot.dispatcher.guardrails import terminal_failure
...
from ghostroot.dispatcher.runtime.containers import ContainerManager
from ghostroot.dispatcher.runtime.startup_healthcheck import format_failure_summary, run_startup_healthchecks
from ghostroot.dispatcher.scheduler.worker_select import choose_worker
from ghostroot.dispatcher.tasks.bootstrap import run_bootstrap_task
from ghostroot.dispatcher.tasks.explore import run_explore_task
from ghostroot.dispatcher.tasks.reason import run_reason_task
from ghostroot.dispatcher.tasks.report import run_report_task
from ghostroot.server.models import Intent, ProjectDetail, ProjectReport, ProjectSummary
...
class DispatcherLoop:
    def __init__(self, config_path: Path):
        ...
        self.client = GhostrootClient(self.config.server)
    ...
    def _dispatch_available(self, summaries: list[ProjectSummary]) -> None:
        ...
        if not active:
            self._log_changed("dispatch/global", logging.INFO, "skip active dispatch because no active projects")
            self._dispatch_pending_reports(summaries)
            return
    ...
    def _try_dispatch_project(self, summary: ProjectSummary) -> bool:
        ...
        if project.project.status != "active":
            ...
            return False
        if self._stop_project_on_terminal_failure(project):
            return False
        if self._is_initial_project(project):
            ...
            export_yaml = self.client.export_project_for_model_context(summary.id)
    ...
    def _stop_project_on_terminal_failure(self, project: ProjectDetail) -> bool:
        failure = terminal_failure(project)
        if failure is None:
            return False
        response = self.client.update_project_status(project.project.id, "stopped")
        if response.ok or response.status_code in (403, 409):
            LOG.warning(
                "stopped project due to terminal target failure project=%s fact=%s reason=%s status=%s",
                project.project.id,
                failure.fact_id,
                failure.reason,
            ...
            return True
        LOG.warning(
            "failed to stop project after terminal target failure project=%s fact=%s reason=%s status=%s body=%s",
            project.project.id,
            failure.fact_id,
            failure.reason,
            response.status_code,
    ...
    def _dispatch_pending_reports(self, summaries: list[ProjectSummary]) -> None:
        completed = self._ordered_projects([summary for summary in summaries if summary.status == "completed"])
        if not completed:
            return
        for summary in completed:
            ...
            if self._project_running_task_count(summary.id) >= self.config.runtime.max_project_workers:
                continue
            if self._project_has_running_report(summary.id):
                continue
            try:
                reports = self.client.list_reports(summary.id)
            except requests.RequestException as exc:
                LOG.warning("list reports failed project=%s error=%s", summary.id, exc)
                continue
            pending = [report for report in reports if report.status == "pending"]
            if not pending:
                continue
            pending.sort(key=lambda report: (report.created_at, report.id), reverse=True)
            if self._dispatch_report(summary.id, pending[0]):
                return

    def _dispatch_report(self, project_id: str, report: ProjectReport) -> bool:
        selection = self._select_worker(project_id, "report")
        worker = selection.worker
        if worker is None:
            self._log_changed(
                f"project:{project_id}:worker:report",
                logging.INFO,
                "no worker available for report project=%s report=%s blocked_busy=%s blocked_unhealthy=%s blocked_rejected=%s blocked_task_type=%s",
                project_id,
                report.id,
                selection.blocked_busy,
                selection.blocked_unhealthy,
            ...
            return False
        self._clear_log_state(f"project:{project_id}:worker:report")
        claim = self.client.claim_report(project_id, report.id, worker.name)
        if claim.status_code in (403, 409):
            level = logging.INFO if claim.status_code == 403 else logging.WARNING
            LOG.log(level, "report claim failed project=%s report=%s worker=%s status=%s", project_id, report.id, worker.name, claim.status_code)
            return False
        if not claim.ok:
            LOG.warning("report claim failed project=%s report=%s worker=%s status=%s", project_id, report.id, worker.name, claim.status_code)
            return False
        try:
            future = self.executor.submit(
                run_report_task,
            ...
            LOG.exception("failed to submit report task project=%s report=%s worker=%s", project_id, report.id, worker.name)
            self._best_effort_fail_report(project_id, report.id, worker.name, "failed to submit report task")
            return False
        self.futures[future] = RunningTask(project_id, "report", worker.name, cancellation, intent_id=report.id)
        self._clear_project_log_state(project_id)
        LOG.info("dispatched report project=%s report=%s worker=%s", project_id, report.id, worker.name)
        return True
    ...
    def _project_has_running_report(self, project_id: str) -> bool:
        return any(task.project_id == project_id and task.task_type == "report" for task in self.futures.values())
    ...
    def _best_effort_fail_report(self, project_id: str, report_id: str, worker_name: str, error: str) -> None:
        response = self.client.fail_report(project_id, report_id, worker_name, error)
        if not response.ok and response.status_code not in (403, 409):
            LOG.warning(
                "report fail write failed project=%s report=%s worker=%s status=%s",
                project_id,
                report_id,
                worker_name,
                response.status_code,
