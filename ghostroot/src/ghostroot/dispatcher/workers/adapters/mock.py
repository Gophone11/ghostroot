...
from ghostroot.dispatcher.config import WorkerConfig, resolve_mock_behavior
from ghostroot.dispatcher.workers.base import DriverResult, SeedSessionDriver
    ...
    if outcome=="report":
        print(json.dumps({"accepted":True,"data":{
            "attack_path_summary":[{
                ...
                "intent_ids":[],
                "result_fact":"goal",
                "why_it_matters":"mock report path"
            }],
            "poc_markdown":"# Mock 渗透报告\\n\\n1. 使用记录中的 origin 作为起点完成验证。（来源：origin）",
            "confidence":"low",
            "gaps":["mock reporter does not inspect full evidence"]
        }}, ensure_ascii=False))
    elif outcome=="rejected":
        print(json.dumps({"accepted":False,"reason":"mock_rejected"}, ensure_ascii=False))
    else:
