...
from ghostroot.dispatcher.models import ReasonCheckpoint
...
from ghostroot.server.models import ProjectDetail, ProjectSummary, Settings
...
class InProcessClient:
    ...
    def export_project_for_model_context(self, project_id: str) -> str:
        response = self.http.get(f"/projects/{project_id}/export?format=yaml&include_tool_events=false")
        response.raise_for_status()
    ...
    def conclude(
        ...
        *,
        ...
        if atoms is not None:
            payload["atoms"] = atoms
        return self._post(f"/projects/{project_id}/intents/{intent_id}/conclude", payload)
