...
def create_intent(project_id: str, body: CreateIntentRequest):
    with get_conn() as conn:
        ...
        conn.execute(
            "INSERT INTO intents (id, project_id, to_fact_id, description, kind, creator, worker, last_heartbeat_at, created_at, concluded_at) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, NULL)",
            (
                iid,
                project_id,
                body.description,
...
def conclude(project_id: str, intent_id: str, body: ConcludeRequest):
    with get_conn() as conn:
        ...
        fact_description = structured_fact_description(
            body.description,
            kind=body.kind,
            ...
            next_policy=body.next_policy,
        )

        conn.execute(
            "INSERT INTO facts (id, project_id, description, kind, outcome, goal_relevance, next_policy, tags_json, atoms_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                fid,
                project_id,
                fact_description,
                ...
                json.dumps(body.atoms, ensure_ascii=False) if body.atoms else None,
            ),
        )
        conn.execute(
            "UPDATE intents SET to_fact_id = ?, worker = ?, stop_condition = ?, last_heartbeat_at = ?, concluded_at = ? WHERE id = ? AND project_id = ?",
            (fid, body.worker, body.stop_condition, now, now, intent_id, project_id),
        )
