...
@router.post("/projects", response_model=ProjectDetail, status_code=201, response_model_exclude_defaults=True)
...
@router.get("/projects/{project_id}", response_model=ProjectDetail, response_model_exclude_defaults=True)
...
@router.get("/projects/{project_id}/tool-events", response_model=list[ToolEvent])
def list_tool_events(project_id: str):
    with get_conn() as conn:
        ...
        return [tool_event_to_model(row) for row in rows]


@router.get("/projects/{project_id}/metrics", response_model=ProjectMetrics)
def get_project_metrics(project_id: str):
    with get_conn() as conn:
        return build_project_metrics(conn, project_id)


@router.post("/projects/{project_id}/tool-events", response_model=list[ToolEvent])
def record_tool_events(project_id: str, body: RecordToolEventsRequest):
    with get_conn() as conn:
        get_project_or_404(conn, project_id)
        now = utcnow()
...
@router.post("/projects/{project_id}/complete", response_model=Intent, response_model_exclude_defaults=True)
def complete_project(project_id: str, body: CompleteRequest):
    with get_conn() as conn:
        ...
        conn.execute(
            "INSERT INTO intents (id, project_id, to_fact_id, description, kind, stop_condition, creator, worker, last_heartbeat_at, created_at, concluded_at) VALUES (?, ?, 'goal', ?, ?, ?, ?, ?, ?, ?, ?)",
            (iid, project_id, body.description, "complete_check", "complete", body.worker, body.worker, now, now, now),
        )
...
@router.post("/projects/{project_id}/reopen", response_model=ReopenResponse, response_model_exclude_defaults=True)
def reopen_project(project_id: str, body: ReopenRequest):
    with get_conn() as conn:
        ...
        conn.execute(
            "INSERT INTO facts (id, project_id, description, kind, outcome, goal_relevance, next_policy) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (fact_id, project_id, description, "human_feedback", "corrective", "rules_out", "branch"),
        )
        conn.execute(
            "INSERT INTO intents (id, project_id, to_fact_id, description, kind, stop_condition, creator, worker, last_heartbeat_at, created_at, concluded_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (intent_id, project_id, fact_id, "external_feedback", "complete_check", "corrective", creator, creator, now, now, now),
        )
