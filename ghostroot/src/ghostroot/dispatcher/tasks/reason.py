...
from ghostroot.dispatcher.contracts import parse_json_output, validate_reason_payload
...
from ghostroot.dispatcher.prompting import (
    format_fact_ids,
    format_open_intents,
    load_prompt,
    render_prompt,
...
from ghostroot.dispatcher.tasks.common import (
...
from ghostroot.server.models import ProjectDetail
...
def run_reason_task(
    ...
    try:
        ...
        if failure is not None:
            response = client.update_project_status(project.project.id, "stopped")
            if not response.ok and response.status_code not in (403, 409):
                LOG.warning(
                    "reason terminal guard could not stop project project=%s fact=%s reason=%s status=%s body=%s",
                    project.project.id,
                    failure.fact_id,
                    failure.reason,
                ...
                LOG.warning(
                    "reason terminal guard stopped project project=%s fact=%s reason=%s status=%s",
                    project.project.id,
                    failure.fact_id,
                    failure.reason,
                    response.status_code,
        ...
        result = run_worker_process(
            ...
            lease=lease,
            cancellation=cancellation,
            telemetry_project_id=project.project.id,
            telemetry_task_type="reason",
        )
        record_tool_events(client, project.project.id, result)
        ...
        if kind == "complete":
            ...
            return "success"
        if kind == "intents":
            if config.tasks.reason.filter_intents:
                data = filter_reason_intents(project, data)
                if not data:
                    LOG.info(
                        "reason finished without graph change after filtering branch-expanding intents project=%s worker=%s execute_ms=%s total_ms=%s",
                        project.project.id,
                        worker.name,
                        execute_ms,
                        total_ms,
