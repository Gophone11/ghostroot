...
TaskType = Literal["reason", "explore", "bootstrap", "report"]
...
MOCK_ALLOWED_OUTCOMES: dict[str, frozenset[str]] = {
    ...
    "report": frozenset({"report", "rejected", "invalid_json", "invalid_payload", "command_fail"}),
}
...
class TasksConfig(BaseModel):
    ...
    report: ReportTaskConfig = Field(default_factory=lambda: ReportTaskConfig(timeout=180))
...
def validate_prompt_resources(prompt_group: str) -> None:
    prompts_dir = resources.files("ghostroot.dispatcher.prompts")
