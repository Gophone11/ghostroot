...
from ghostroot.dispatcher.workers.adapters._curl import build_verbose_curl_healthcheck, expand_env, render_curl_command
from ghostroot.dispatcher.workers.base import DriverResult, RegexSessionDriver
    ...
    def build_execute(self, worker: WorkerConfig, prompt: str, session: str | None) -> DriverResult:
        ...
        return DriverResult(
            argv=[
                ...
                env["CODEX_MODEL"],
                "-c",
                'model_provider="ghostroot"',
                "-c",
                'model_providers.ghostroot.name="ghostroot"',
                "-c",
                'model_providers.ghostroot.wire_api="responses"',
                "-c",
                'model_reasoning_effort="high"',
                "-c",
                f'model_providers.ghostroot.base_url="{env["CODEX_BASE_URL"]}"',
                "-c",
                'model_providers.ghostroot.env_key="OPENAI_API_KEY"',
                "--",
                prompt,
            ]
        )
