...
from ghostroot.dispatcher.config import DispatchConfig, WorkerConfig, validate_prompt_resources
from ghostroot.dispatcher.workers.adapters.codex import CodexDriver
...
def test_default_fact_prompts_define_polarity_and_goal_proof_precedence() -> None:
    prompt_dir = Path("src/ghostroot/dispatcher/prompts/default")
    for name in ("bootstrap.md", "bootstrap_conclude.md", "explore.md", "explore_conclude.md"):
        text = (prompt_dir / name).read_text(encoding="utf-8")
        assert "Atom `polarity` is not a truth flag" in text
        assert "empty replies" in text
        assert "the fact must be `kind: goal_proof`" in text
        assert "must not downgrade the goal proof" in text
...
def test_pi_driver_models_json_and_execute_argv_include_context_window_and_tools() -> None:
    ...
    assert models["providers"]["ghostroot"]["models"][0]["contextWindow"] == 131072
...
def test_codex_driver_execute_argv_passes_model_endpoint_and_prompt() -> None:
    ...
    assert 'model_providers.ghostroot.base_url="http://api/v1"' in argv
