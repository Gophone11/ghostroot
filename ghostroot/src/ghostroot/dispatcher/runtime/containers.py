...
from ghostroot.dispatcher.config import ContainerConfig
from ghostroot.dispatcher.runtime.process import ManagedProcess

LOG = logging.getLogger(__name__)
TELEMETRY_ROOT = "/tmp/ghostroot-telemetry"
TELEMETRY_BIN = f"{TELEMETRY_ROOT}/bin"
TELEMETRY_EVENTS = f"{TELEMETRY_ROOT}/events.jsonl"
TELEMETRY_TOOLS = (
    "curl",
    "wget",
    "python",
...
class ContainerManager:
    _PREFIX = "ghostroot-dispatch-"
    _STARTUP_PREFIX = "ghostroot-startup-healthcheck-"
    ...
    def ensure_tool_telemetry(self, container_name: str) -> None:
        container = self._require_container(container_name)
        try:
            result = container.exec_run(["/bin/sh", "-lc", self._tool_telemetry_install_script()])
        except DockerException as exc:
            raise RuntimeError(f"failed to install tool telemetry in {container_name}: {exc}") from exc
        ...
        if exit_code not in (0, None):
            ...
            raise RuntimeError(f"failed to install tool telemetry in {container_name}: {output_text}")

    def collect_tool_events(self, container_name: str) -> str:
        container = self._require_container(container_name)
        try:
            result = container.exec_run(["/bin/sh", "-lc", f"test -f {TELEMETRY_EVENTS} && cat {TELEMETRY_EVENTS} || true"])
        except DockerException as exc:
            LOG.warning("failed to collect tool telemetry container=%s error=%s", container_name, exc)
            return ""
        output = getattr(result, "output", b"")
        if isinstance(output, bytes):
    ...
    @staticmethod
    def _tool_telemetry_install_script() -> str:
        tools = " ".join(TELEMETRY_TOOLS)
        return f"""\
set -eu
ROOT={TELEMETRY_ROOT}
BIN={TELEMETRY_BIN}
...
logger="$(command -v python3 2>/dev/null || command -v python 2>/dev/null || true)"
case "$logger" in
  "$BIN"/*) logger="" ;;
esac
for tool in {tools}; do
  real="$(command -v "$tool" 2>/dev/null || true)"
  if [ -z "$real" ] || [ "$real" = "$BIN/$tool" ]; then
    continue
  fi
  case "$real" in
    "$BIN"/*) continue ;;
...
REAL="$real"
LOGGER="$logger"
EVENT_FILE="\\${{GHOSTROOT_TOOL_EVENT_FILE:-{TELEMETRY_EVENTS}}}"
mkdir -p "\\$(dirname "\\$EVENT_FILE")" 2>/dev/null || true
if [ -n "\\$LOGGER" ]; then
  "\\$LOGGER" - "\\$EVENT_FILE" "\\$TOOL" "\\$@" <<'PY' 2>/dev/null || true
import json
import os
import shlex
import sys
...
args = sys.argv[3:]
event = {{
    "event_id": "%s-%s-%s-%s" % (time.time_ns(), os.getpid(), os.getppid(), tool),
    "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    "project_id": os.environ.get("GHOSTROOT_PROJECT_ID", ""),
    "task_type": os.environ.get("GHOSTROOT_TASK_TYPE", ""),
    "phase": os.environ.get("GHOSTROOT_PHASE", ""),
    "intent_id": os.environ.get("GHOSTROOT_INTENT_ID", ""),
    "worker": os.environ.get("GHOSTROOT_WORKER", ""),
    "task_run_id": os.environ.get("GHOSTROOT_TASK_RUN_ID", ""),
    "tool": tool,
